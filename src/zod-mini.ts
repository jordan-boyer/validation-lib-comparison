...
for (let i = 0; i < nbIterations; i++) {
  const userSchema = object({
    name: string(),
    age: optional(number()),
    phone: optional(string().check(
      refine((phone) =>
        typeof phone === "number" ? String(phone) : phone
      ),
    ))
  });
  ...
  // @ts-expect-error Issue with zod4-mini : because no default value, the output type is messed up
  checkUserA(userA);
