...
Bun.write("./metrics/suretype.heap.before.json", JSON.stringify(heapStats()));
...
for (let i = 0; i < nbIterations; i++) {
  ...
  const ensureUser = compile(userSchema, {
    ensure: true,
...
Bun.write("./metrics/suretype.heap.after.json", JSON.stringify(heapStats()));
